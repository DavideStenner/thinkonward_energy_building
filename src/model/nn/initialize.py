...
from src.nn.light_module import TablePredictor
from src.utils.logging_utils import get_logger


class TabularFFInit(ModelInit):
    def __init__(self, 
            experiment_name:str, 
    ...
    def initialize_logger(self) -> None:
        self.training_logger: logging.Logger = get_logger(
            file_name='training_nn.txt', path_logger=self.experiment_path
        )
    ...
    def get_model_file_name_dict(self) -> None:
        self.model_file_name_dict: dict[str, str] =  {
            ...
            'model_pickle_list': {
                type_model: f'model_{type_model}_list.pkl'
                for type_model in self.model_used
            },
            'model_list': {
                type_model: f'model_{type_model}' + '_{fold_}.ckpt'
                for type_model in self.model_used
            }
        }
    ...
    def save_pickle_model_list(self, model_list: list[L.LightningModule], type_model: str) -> None:
        with open(
            os.path.join(
                self.experiment_type_path.format(type=type_model),
                'model',
    ...
    def load_pickle_model_list(self, type_model: str) -> list[L.LightningModule]:
        with open(
            os.path.join(
                self.experiment_type_path.format(type=type_model),
                'model',
    ...
    def load_model_list(self, type_model: str, file_name: str) -> list[L.LightningModule]:
        
        return [
            TablePredictor.load_from_checkpoint(
                os.path.join(
                    self.experiment_type_path.format(type=type_model),
                    'model',
